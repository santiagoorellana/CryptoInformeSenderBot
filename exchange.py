...
class Exchange(Basics):

    def __init__(self, exchangeId:str, apiKey:str, secret:str):
    ...
    def select_exchange(self, exchangeId:str, apiKey:str, secret:str) -> bool:
        ...
        try:
            ...
            log.info(f"ERROR: No se pudo seleccionar el exchange: {exchangeId}")
    ...
    def _check_exchange_method(self, method:str) -> bool:
        ...
        if not self.exchange.has[method]:
            log.info(f'ERROR: El exchange {self.exchangeId} no tiene el metodo "{method}".')
            return False
        else:
            return True
    ...
    def check_exchange_methods(self, checkPrivate:bool) -> bool:
        ...
        try:
            ...
            log.info(f"ERROR: Verificando los metodos del exchange.")
            log.info(f"EXCEPTION: {str(e)}")
            return False
    ...
    def load_markets_and_currencies(self) -> bool:
        ...
        log.info(f"ERROR: Cargando datos de los mercados y sus cryptomonedas.")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return False
    ...
    def get_balance(self) -> Optional[Balance]:
        ...
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Cargando el balance de la cuenta.")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
    ...
    def get_tickers(self, symbols:Optional[ListOfMarketsId]=None) -> Optional[DictOfTickers]:
        ...
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Obteniendo los tickers de todos los mercados.")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
    ...
    def get_ticker(self, symbol:MarketId) -> Optional[Ticker]:
        ...
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Obteniendo el ticker del mercado {symbol}")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
    ...
    def execute_market_buy(self, symbol:MarketId, amountAsBase:float, params:Dict) -> Optional[Order]:
        ...
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Comprando {amountAsBase} a precio de mercado en {symbol}")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
    ...
    def execute_market_sell(self, symbol:MarketId, amountAsBase:float, params:Dict) -> Optional[Order]:
        ...
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Vendiendo {amountAsBase} a precio de mercado en {symbol}")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
    ...
    def get_order(self, orderId:str, symbol:MarketId) -> Optional[Order]:
        ...
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Obteniendo la orden {orderId} en {symbol}")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
    ...
    def cancel_order(self, orderId:str, symbol:MarketId) -> Optional[Order]:
        ...
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Cancelando orden {orderId} en {symbol}")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
    ...
    def get_last_candles(self, symbol:MarketId, count:int=24, timeFrame:str='1h') -> Optional[ListOfCandles]:
        ...
        if timeFrame is None:
            log.info(f"ERROR: El timeFrame {timeFrame} no esta soportado por el exchange.")
            return None
        exceptionMsg = ""
        for i in range(self.insistenceCountMax):
            try:
                ...
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Obteniendo las ultimas {count} velas {timeFrame} del mercado {symbol}")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None


        
    def get_order_book(self, symbol:MarketId, count:int=10) -> Optional[Dict]:
        '''
        Obtiene el order book
        Si se produce un error, lo intenta nuevamente varias veces antes de fallar.
        param symbol: Mercado al que se le va a leer el order book.
        param count: Cantidad de datos hacia atras que se deben buscar.
        return: Devuelve un objeto con los datos del bids y asks. Si falla devuelve None.
        '''
        exceptionMsg = ""
        for i in range(self.insistenceCountMax):
            try:
                return self.exchange.fetch_order_book(symbol, count)
            except Exception as e:
                exceptionMsg = str(e)
                time.sleep(self.insistencePauseSeconds) 
        log.info(f"ERROR: Obteniendo el order book del mercado {symbol}")
        log.info(f"EXCEPTION: {exceptionMsg}")
        return None
...
#x = Exchange('hitbtc', "", "")
#print(x.get_last_candles("BTC/USDT"))
